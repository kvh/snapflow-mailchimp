...
def test():
    ...
    try:
        ...
        members_df = pd.DataFrame.from_records([
            dict(email_address="e1@scaledscience.com", status="subscribed", merge_fields={"f1":"v1"}),
            dict(email_address="e2@scaledscience.com", status="subscribed", merge_fields={"f1":"v2"}),
            dict(email_address="e3@scaledscience.com", status="subscribed", merge_fields={"f1":"v3"}),
        ])
        ...
        produce(export_aud, node_timelimit_seconds=5, modules=[snapflow_mailchimp])
